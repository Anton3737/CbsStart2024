...
-- Отримати імена покупців та співробітників, у яких TotalPrice товару більше 1000
SELECT
    (SELECT c.FName
     FROM Customers c
     WHERE c.CustomerNo = o.FK_Cust) AS CustomerName,
    (SELECT e.EmployeeName
     FROM Employees e
     WHERE e.EmployeeNo = o.FK_Emp) AS EmployeeName,
    o.TotalPrice
FROM
    OrderDetails o
WHERE
