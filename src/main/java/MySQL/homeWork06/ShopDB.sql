...
-- Індекс на колонку FName для прискорення пошуку за іменем
CREATE INDEX idx_fname ON Customers (FName);

-- Індекс на колонку Address для швидкого пошуку за місцем проживання
CREATE INDEX idx_address ON Customers (Address);


-- Вибірка імен покупців із Києва
SELECT FName, LName
FROM Customers
WHERE Address = 'Київ';

-- Аналіз продуктивності запитів
EXPLAIN SELECT FName, LName
FROM Customers
WHERE Address = 'Київ';
